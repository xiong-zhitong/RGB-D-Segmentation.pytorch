...
from models.dcn.modules.deform_conv import DeformConvPack
#from dcn.modules.modulated_deform_conv import ModulatedDeformConvPack
import pdb


def calc_max_match(mat0, mat1):
    N,C,H,W = mat0.shape 
...
class RedNet(nn.Module):
    def __init__(self, num_classes=40, pretrained=True):
        ...
        block = Bottleneck
        deformable_block = DeformableBottleneck
        ...
        self.layer4 = self._make_layer(deformable_block, 512, layers[3], stride=2)
        ...
        self.layer4_d = self._make_layer(deformable_block, 512, layers[3], stride=2)
    ...
    def _load_resnet_pretrained(self):
        ...
        #pretrain_dict = torch.load('/home/cmu/RGB-D-Segmentation.pytorch/models/resnet50_19c8e357.pth')
        model_dict = {}
    ...
    def forward_downsample(self, rgb, depth):
        ...
        for i, layer in enumerate(self.layer4):
        	if i==0:
        		offset, x = layer(fuse3)
        		offsets.append(offset)
        	else:
        		offset, x = layer(x)
        		offsets.append(offset)
        #depth = self.layer4_d(depth)
        for i, layer in enumerate(self.layer4_d):
        	offset, depth = layer(depth, offsets[i])
        fuse4 = x + depth
    ...
    def forward_upsample(self, fuse0, fuse1, fuse2, fuse3, fuse4):
        ...
        #out2_up = F.interpolate(out2,scale_factor=2, mode='bilinear')
        #out3_up = F.interpolate(out3,scale_factor=4, mode='bilinear')
        #out_f = torch.stack([out, out2_up, out3_up],2) #N,H,W,3
        #out_f = out_f.sum(2)
...
class DeformableBottleneck(nn.Module):
    ...
    def forward(self, x, offset = None):
        ...
        if offset is None:
        	offset, out = self.conv2(out)
        else:
        	offset, out = self.conv2(out, offset)
        out = self.bn2(out)
        out = self.relu(out)
