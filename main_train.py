...
#from dataset.segmentation_data import *
...
image_h = 480
def train():
    train_data = SUNRGBD(transform=transforms.Compose([scaleNorm(),
                                                     RandomScale((1.0, 1.4)),
    ...
    if args.last_ckpt:
        model = DFSeg_model.RedNet(pretrained=False)
    else:
        model = DFSeg_model.RedNet(pretrained=True)
